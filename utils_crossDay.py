'''
1、本代码包含:
    特征提取、系统时间获取、trigger生成与数据切割、信噪比计算、当天SVM模型训练与测试，隔天深度森林训练与测试
2、已适配stand2姿态
3、label_preprocess:
    trigger识别更新为 寻找离原始Excel表里时间戳最近的一个数据包所在的位置
4、data_preprocess:
    优化了切割数据，避免采集结束后被试做的动作对切割造成干扰
5、deep_forest_online_train:
    如果深度森林模型已存在，新模型将会覆盖旧模型
'''
...
path = basedir + '/libsvm-3.24/python'
...
def label_preprocess(path, subject, num_train):
    ...
    #     a = trigger_data[2 * num + 1] - Trigger[0] + ges_start_time + 250
    #     trig_dyn[num, 0] = a // 100 * 100
    #     b = a + 1000
    #     trig_dyn[num, 1] = b // 100 * 100
    #     trig_dyn[num, 2] = randIndex[0][num%num_gesture]
...
def data_preprocess(path, subject, num_train):
    num_gesture = 5
